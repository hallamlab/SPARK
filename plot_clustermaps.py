...
for t in ['Phylum_plot', 'Class_plot', 'Order_plot', 'Family_plot',
    ...
    pivot_df.to_csv(os.path.join(data_dir, f"spark_old_output/diversity/clustermap_{t}.tsv"), sep='\t')
...
bubble_df = asv_mito_meta_df.groupby(['sample', 'ASV_ID', 'type_group', 'status', 'kit'])['count'].sum().reset_index()
pivot_df = bubble_df.pivot(index='sample', columns='ASV_ID', values='count').fillna(0)

# Map sample to sample_type
col_meta = bubble_df.drop_duplicates('sample')[['sample', 'type_group', 'status', 'kit']].set_index('sample')
...
pivot_df = asv_mito_meta_df.groupby(['sample', 'ASV_ID'])['count'
...
plt.savefig(os.path.join(data_dir, f"spark_old_output/mito/diversity/clustermap_ASV_code_mito.svg"), bbox_inches='tight')
plt.savefig(os.path.join(data_dir, f"spark_old_output/mito/diversity/clustermap_ASV_code_mito.pdf"), bbox_inches='tight')
plt.close()
...
plt.savefig(os.path.join(data_dir, f"spark_old_output/mito/diversity/clustermap_ASV_clustered_mito.svg"), bbox_inches='tight')
plt.savefig(os.path.join(data_dir, f"spark_old_output/mito/diversity/clustermap_ASV_clustered_mito.pdf"), bbox_inches='tight')
plt.close()
