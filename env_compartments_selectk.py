...
Pick the *smallest* K that satisfies:
  - ICL within --select-delta of the best (min) ICL,
...
Notes
-----
- This script is intentionally narrower than your episodic/persistent pipeline:
  it is a "K selection + sanity" gate to stop over-fragmentation before adding
  temporal stickiness / episodic smoothing.
"""
...
def plot_stability(metrics: pd.DataFrame, outpath: str) -> None:
    plt.figure(figsize=(8, 4.5))
...
def plot_min_cluster_frac(metrics: pd.DataFrame, outpath: str) -> None:
    plt.figure(figsize=(8, 4.5))
    plt.plot(metrics["K"], metrics["min_cluster_frac"], marker="o")
    plt.axhline(y=np.nan, linewidth=0)  # no-op; keeps consistent
    plt.xlabel("K")
...
def select_k(metrics: pd.DataFrame, cfg: Config) -> Tuple[int, pd.DataFrame]:
    ...
    if cfg.select_by == "icl":
        best = float(m.loc[m["ICL"].idxmin(), "ICL"])
        m["within_delta"] = (m["ICL"] - best) <= cfg.select_delta
        # lower is better
        eligible = m[m["within_delta"] & m["passes_min_cluster"] & m["passes_stability"]]
        if eligible.empty:
            # relax: keep within_delta only
            eligible = m[m["within_delta"]]
        if eligible.empty:
            eligible = m
        chosen = int(eligible.sort_values(["K"]).iloc[0]["K"])
    elif cfg.select_by == "bic":
        best = float(m.loc[m["BIC"].idxmin(), "BIC"])
        m["within_delta"] = (m["BIC"] - best) <= cfg.select_delta
        eligible = m[m["within_delta"] & m["passes_min_cluster"] & m["passes_stability"]]
        if eligible.empty:
            eligible = m[m["within_delta"]]
        if eligible.empty:
        ...
        best = float(m["CV_loglik_mean"].max())
        m["within_delta"] = (best - m["CV_loglik_mean"]) <= cfg.select_delta
        eligible = m[m["within_delta"] & m["passes_min_cluster"] & m["passes_stability"]]
        if eligible.empty:
            eligible = m[m["within_delta"]]
        if eligible.empty:
    ...
    decision_cols = [
        ...
        "passes_min_cluster", "passes_stability", "within_delta",
    ]
...
def main() -> None:
    ...
    plot_stability(metrics, os.path.join(plots_dir, "selectk_stability_ari.png"))
    plot_min_cluster_frac(metrics, os.path.join(plots_dir, "selectk_cluster_sizes.png"))
    ...
    ent_total, ent_mean = responsibilities_entropy(resp_sel)
