...
asv_array = multiplicative_replacement(asv_table.values)
# Apply centered log-ratio
clr_transformed = clr(asv_array)
# Optional: convert back to DataFrame
clr_df = pd.DataFrame(clr_transformed, index=asv_table.index, columns=asv_table.columns)

sample_metadata = metadata_df.loc[shared_samples]

outliers_df = ensemble_outlier_detection(clr_df, sample_metadata, group_col='keep_col').reset_index()
outliers_df.to_csv(os.path.join(data_dir, 'methods_output/metadata/outliers_table.tsv'), sep='\t', index=False)

outliers_df = ensemble_outlier_detection(clr_df, sample_metadata, group_col='type_group').reset_index()
outliers_df.to_csv(os.path.join(data_dir, 'methods_output/metadata/outliers_type_group.tsv'), sep='\t', index=False)
