...
outliers_df.to_csv(os.path.join(data_dir, 'spark_old_output/metadata/outliers_table.tsv'), sep='\t', index=False)

outliers_df = ensemble_outlier_detection(clr_df, sample_metadata, group_col='type_group').reset_index()
outliers_df.to_csv(os.path.join(data_dir, 'spark_old_output/metadata/outliers_type_group.tsv'), sep='\t', index=False)
