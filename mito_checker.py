...
mitomaster_full_file = os.path.join(data_dir, 'spark_methods_output_tester/mito/mitomap/mitomaster_combined.tsv')
mito_blast_file = os.path.join(data_dir, 'spark_methods_output_tester/mito/mitomap/mito_ncbi.blast6.tsv')
gg2_full_file = os.path.join(data_dir, 'spark_methods_output_tester/taxonomy/ASV_SILVA_tax.full-length.vsearch.tsv')
biof_file = os.path.join(data_dir, 'spark_methods_output_tester/mito/mitomap/ssu_pipeline_contaminants.blast6.tsv')
...
mmfl_df = pd.DataFrame(lines_list[1:], columns=['Sequence_ID', 'haplo'])
mmfl_df['Sequence_ID'] = mmfl_df['Sequence_ID'].str.split(';').str[0]
...
mibl_df['qseqid'] = mibl_df['qseqid'].str.split(';').str[0]

ggfl_df = pd.read_csv(gg2_full_file, header=0, sep='\t')
ggfl_df['Sequence_ID'] = ggfl_df['Feature ID'].str.split(';').str[0]
...
biof_df['qseqid'] = biof_df['qseqid'].str.split(';').str[0]
...
master_df['MITOMASTER'] = [0 if x in list(mmfl_df['Sequence_ID']) else 1 for x in master_df['Sequence_ID']]
...
master_df.to_csv(os.path.join(data_dir, 'spark_methods_output_tester/mito/mitomap/nontarget.master.tsv'), sep='\t', index=False)
...
plt.tight_layout()
plt.savefig(os.path.join(data_dir, "spark_methods_output_tester/mito/mitomap/non-target_plot.svg"))
plt.savefig(os.path.join(data_dir, "spark_methods_output_tester/mito/mitomap/non-target_plot.pdf"))
plt.close()
...
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
plt.savefig(os.path.join(data_dir, "spark_methods_output_tester/mito/mitomap/Microbial_Host_plot.svg"))
plt.savefig(os.path.join(data_dir, "spark_methods_output_tester/mito/mitomap/Microbial_Host_plot.pdf"))
plt.close()
...
plt.xticks(rotation=45, ha='right')
plt.tight_layout()
plt.savefig(os.path.join(data_dir, "spark_methods_output_tester/mito/mitomap/Non-Target_barplot.svg"))
plt.savefig(os.path.join(data_dir, "spark_methods_output_tester/mito/mitomap/Non-Target_barplot.pdf"))
plt.close()