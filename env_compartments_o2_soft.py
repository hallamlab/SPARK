...
def parse_args() -> RunConfig:
    ...
    ap.add_argument("--softness-s", type=float, default=2.0,
                    help="Softness width around thresholds in O2 units (default 2.0).")
    ...
    return RunConfig(
        ...
        softness_s=float(ns.softness_s),
...
def soft_o2_memberships(
    ...
    s = float(s)
    if s <= 0:
        raise ValueError("softness_s must be > 0.")

    x = o2.astype(float)

    # Gates: g_i ~ P(O2 >= T_i) with softness s
    g1 = _sigmoid((x - T_oxic_dyso) / s)   # above oxic/dyso boundary
    g2 = _sigmoid((x - T_dyso_sub) / s)    # above dyso/sub boundary
    g3 = _sigmoid((x - T_sub_anox) / s)    # above sub/anox boundary
...
def main() -> None:
    ...
    print(f"     Softness s: {cfg.softness_s}")
    if cfg.episodic_smoothing:
