...
voc_metadata_df = voc_metadata_df[['sample_name', 'sample', 'participant_num',
...
metavoc_df['subclass2'] = np.where(
    (metavoc_df['type_group'] == 'Lung Brush') & metavoc_df['subclass2'].notna(),
    metavoc_df['subclass2'],
    'No VOCs'
)
metavoc_df['subclass2'] = np.where(
    (metavoc_df['type_group'] != 'Lung Brush'),
    np.nan,
    metavoc_df['subclass2']
)

metavoc_df['pass_filter'] = [t if s in list(asv_meta_df['sample']) else 'Failed-QC'
                              for s,t in  zip(metavoc_df['sample'], metavoc_df['subclass2'])
                              ]
metavoc_df = metavoc_df.dropna(subset=['pass_filter'])

long_df = metavoc_df.groupby(['subclass2', 'pass_filter', 'sample'])['raw_count'].sum().reset_index()
long_df = long_df.loc[long_df['raw_count'] > 0] # remove empty values

# Plot
plt.figure(figsize=(10, 10))
ax = sns.boxplot(
    x='subclass2', y='raw_count', data=long_df,
...
sns.stripplot(data=long_df, x='subclass2', y='raw_count',
              hue='pass_filter', alpha=0.75, ax=ax, legend=False,
              jitter=0.25, palette=brush_palette
              )
...
sample_keep_list = list(voc_metadata_df['sample'])
final_asv_df = cleaned_asv_df[[x for x in cleaned_asv_df.columns if x in sample_keep_list]]
...
new_asv_meta_df = asv_meta_df.copy().loc[asv_meta_df['sample'].isin(sample_keep_list)
                  ].merge(voc_metadata_df[['sample', 'subclass2']], how='left', on='sample')
...
new_tax_df = tax_df.copy().loc[tax_df.index.isin(asv_keep_list)]
...
col_colors_df = pd.DataFrame({
    'subclass2': m_df['subclass2'].map(brush_palette),
...
handles = []
for group in ['ctrl-brush', 'ca-contra', 'ca-lung']:
...
ax, tidy = plot_grouppair_violins_sns(
    shared_percent, new_metadata_df,
    sample_id_col="sample",
    group_col="subclass2",
    include_within=True,
    group_order=['ctrl-brush', 'ca-contra', 'ca-lung'],
    group_colors=brush_palette,   # used for within; between are blended
    title="ASVs Shared by Subclass2",
    ylabel="ASVs Shared (%)",
