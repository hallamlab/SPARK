...
def main():
    ...
    # Compute Shannon diversity for each lmp_id from the ASV counts
    shannon_results = {}
    for lmp_id in asv_df.index:
        counts = asv_df.loc[lmp_id].values
        shannon_results[lmp_id] = calc_shannon(counts)
    ...
    # Compute Shannon diversity for each lmp_id from the ASV counts
    shannon_results = {}
    for lmp_id in mito_df.index:
        counts = mito_df.loc[lmp_id].values
        shannon_results[lmp_id] = calc_shannon(counts)
    shannon_df = pd.DataFrame.from_dict(shannon_results, orient="index", columns=["Shannon"])
    shannon_df.index.name = "lmp_id"
    ...
    # Compute pairwise Bray-Curtis distances between lmp_ids
    # Here, each row in asv_df is assumed to be a lmp_id
