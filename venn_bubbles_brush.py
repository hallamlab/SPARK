...
#venn_kit_df = pd.read_csv(os.path.join(data_dir, "spark_combined_output/brush/metadata/venn3_presence_table_kit.tsv"), sep="\t", header=0)
...
for vgrp in venn_df['grouping'].unique():
    ...
    v_spp_df = asv_meta_df.loc[asv_meta_df['ASV_ID'].isin(v_asvs)
                                   ].groupby(['subclass2', 'Family', 'Genus'], dropna=False)['corr_count'].sum().reset_index(
                                    )

    v_spp_df['subclass2'] = pd.Categorical(v_spp_df['subclass2'], ordered_type)
    ...
    sns.scatterplot(data=v_spp_df, x='subclass2', y='Family Genus',
                    size='corr_count', sizes=(5, 500), palette=brush_palette,
                    hue_order=ordered_type, hue='subclass2', alpha=0.75
                    )
    ...
    plt.savefig(os.path.join(data_dir, f'spark_combined_output/brush/metadata/{vgrp_str}_Genus_bubbleplot.svg'))
    plt.savefig(os.path.join(data_dir, f'spark_combined_output/brush/metadata/{vgrp_str}_Genus_bubbleplot.pdf'))
    plt.close()
venn_tax_df = pd.concat(venn_tax_dfs)
venn_tax_df.to_csv(os.path.join(data_dir, 'spark_combined_output/brush/metadata/Three_types_venn_presence_tax.tsv'), sep='\t')