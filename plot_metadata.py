...
metadata_df['type_code'] = [t[0] for t in metadata_df['Type_Group']]
metadata_df['lung_code'] = [l[0] if l[0] in ['R', 'L'] else '' for l in metadata_df['Type']]
metadata_df['sample_code'] = ['_'.join([p, t, l]).rstrip('_') for p,t,l in zip(metadata_df['patient_code'],
                                                                   metadata_df['type_code'],
                                                                   metadata_df['lung_code']
                                                                   )]
...
asv_cla_df = asv_meta_df.groupby(['Type_Group', 'Class', 'sample', 'Sample'])['count'].sum().reset_index()
...
total_class = asv_cla_df.groupby('Class')['count'].sum()
...
asv_df[sub_df['sample']].to_csv(final_asv_path, sep='\t')
...
comparisons = [
    ("Oral Rinse", "BAL"),
    ("BAL", "Lung Brush"),
    ("Oral Rinse", "Lung Brush")
]

# Add annotations:
ax = g.ax  # Extract axis from FacetGrid
annotator = Annotator(ax, comparisons, data=sub_df, x="Type_Group", y="Shannon", order=['Oral Rinse', 'BAL', 'Lung Brush'])
...
sns.scatterplot(data=sub_df, x="UMAP1", y="UMAP2",
                hue="Type_Group", hue_order=['Oral Rinse', 'BAL', 'Lung Brush'],
                size='count', sizes=(40, 400),
                palette=three_palette, style="status",
                alpha=0.75
                )
...
sns.scatterplot(data=sub_df, x="UMAP1", y="UMAP2", hue="Type_Group", size='count', sizes=(40, 400),
                palette=three_palette, alpha=0.75, style='lung_code',
                edgecolor='grey', linewidth=0.5
                )
...
plt.figure(figsize=(12, 10))
sns.scatterplot(data=sub_df, x="UMAP1", y="UMAP2", hue="Type_Group", size='count', sizes=(40, 400),
                palette=three_palette, alpha=0.75, style='Set',
                edgecolor='grey', linewidth=0.5
                )
...
plt.figure(figsize=(12, 10))
sns.scatterplot(data=sub_df, x="UMAP1", y="UMAP2", hue="Type_Group", size='count', sizes=(40, 400),
                palette=three_palette, alpha=0.75, style='patient_code',
                edgecolor='grey', linewidth=0.5
                )
...
for t in type_groups:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/{t}_count_data.svg"))
    plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/{t}_count_data.pdf"))
    plt.close()
...
asv_meta_df = asv_tax_df.merge(metadata_df, on='sample',
                               how='left').reset_index()

asv_meta_df = asv_meta_df.loc[~asv_meta_df['Type_Group'].isin(['Skin Brush', 'Scope Flush'])]
top10 = total_abundance.sort_values(ascending=False).head(25).index.tolist()
top10_class = total_class.sort_values(ascending=False).head(25).index.tolist()
asv_meta_df['Phylum_plot'] = asv_meta_df["Phylum"].apply(lambda x: x if x in top10 else "Other")
asv_meta_df['Class_plot'] = asv_meta_df["Class"].apply(lambda x: x if x in top10_class else "Other")

bubble_df = asv_meta_df.groupby(['sample_code', 'Phylum_plot', 'Type_Group', 'status'])['count'].sum().reset_index()
pivot_df = bubble_df.pivot(index='sample_code', columns='Phylum_plot', values='count').fillna(0)
...
plt.tight_layout()
plt.savefig(os.path.join(data_dir, "vsearch_output/diversity/dendrogram.svg"))
plt.savefig(os.path.join(data_dir, "vsearch_output/diversity/dendrogram.pdf"))
plt.close()
...
plt.xticks(rotation=90)
plt.tight_layout()
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/bubbleplot.svg"))
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/bubbleplot.pdf"))
plt.close()
...
pivot_df = asv_meta_df.groupby(['sample_code', 'Phylum_plot'])['count'
...
# For Type_Group
for group, color in three_palette.items():
...
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap.svg"), bbox_inches='tight')
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap.pdf"), bbox_inches='tight')
plt.close()




pivot_log_sorted = pivot_log[sorted(pivot_log.columns)]
g = sns.clustermap(
    pivot_log_sorted,
    method='ward',
    metric='euclidean',
...
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap_patient.svg"), bbox_inches='tight')
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap_patient.pdf"), bbox_inches='tight')
plt.close()


bubble_df = asv_meta_df.groupby(['sample_code', 'Class_plot', 'Type_Group', 'status'])['count'].sum().reset_index()
pivot_df = bubble_df.pivot(index='sample_code', columns='Class_plot', values='count').fillna(0)
...
pivot_df = asv_meta_df.groupby(['sample_code', 'Class_plot'])['count'
                                ].sum().reset_index().pivot(index='Class_plot',
                                                            columns='sample_code',
                                                            values='count'
                                                            ).fillna(0)
...
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap_Class.svg"), bbox_inches='tight')
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap_Class.pdf"), bbox_inches='tight')
plt.close()
...
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap_ISA.svg"), bbox_inches='tight')
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap_ISA.pdf"), bbox_inches='tight')
plt.close()
...
bubble_df = asv_meta_df.groupby(['sample_code', 'Phylum_plot'])['count'].sum().reset_index()
pivot_df = bubble_df.pivot(index='sample_code', columns='Phylum_plot', values='count').fillna(0)
...
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap_wControls.svg"), bbox_inches='tight')
plt.savefig(os.path.join(data_dir, f"vsearch_output/diversity/clustermap_wControls.pdf"), bbox_inches='tight')
plt.close()
